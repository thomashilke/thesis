...
conjointement le modèle d'écoulement stationnaire évoqué dans le
chapitre \ref{chap:populations} et le modèle de transport et
dissolution d'alumine décrit dans le chapitre \ref{chap:fourier} par
l'abréviation F3D. Dans cette partie, on considérera uniquement une
dissolution des particules d'alumine indépendante de la température,
c'est-à-dire que le champ $c^\mathrm{F3D}$ est calculé avec le modèle
3D standard en désactivant la dépendance de la vitesse de dissolution
en fonction de la température du bain.
