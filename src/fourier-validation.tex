...
\Lambda\times(0,\thickness)$. On vérifie que les fonctions $u_1, u_2,
u_3:\Omega\to\mathbb R$ et $p:\Omega\to\mathbb R$ définies par:
\begin{align}
...
On choisit dans cette partie une viscosité scalaire unité,
c'est-à-dire que $\mu_{i,j} = 1\ \forall i,j$. On s'intéresse d'une
part à la convergence de l'erreur d'approximation de $u_{h,K}$ donnée
par (\ref{eq:u-h-12}),(\ref{eq:u-h-3}) vers la solution exacte
